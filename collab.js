(function(){
  ...
  function acceptanceItem(kind,title,item,action){const file=item?.file,needsFile=title!=='需求方案',ready=!!item&&(!needsFile||!!file),detail=!item?'尚未提交':file?.name||(needsFile?'缺少上传文件':'在线需求内容');return `<article class="acceptance-item ${ready?'ready':item?'partial':'missing'}"><i>${ready?'✓':item?'!':'○'}</i><div><small>${kind}</small><b>${title}</b><span>${esc(detail)}${item?.version?` · ${esc(item.version)}`:''}</span></div><div>${item?`<button class="secondary" data-collab="${action}">查看内容</button>${file?.id?`<button class="link" data-collab="downloadFile" data-file-id="${file.id}">下载</button>`:''}`:'<em>等待提交</em>'}</div></article>`}
  function acceptancePanel(){
    if(!collab.requirement)return '<section class="acceptance-card acceptance-empty"><div class="acceptance-waiting"><b>客户验收</b><span>需求提交后，这里会集中显示需求方案、实施文档、开发代码和测试入口。</span></div></section>';
    const status=collab.acceptance?.status;
    const complete=!!(collab.requirement&&collab.implementation?.file&&collab.code?.file);
    const testUrl=collab.code?.testUrl||'';
    const isCustomer=collab.role==='customer';
    const acceptanceAction=status==='accepted'
      ?`<div class="acceptance-result"><b>验收通过</b><span>${collab.acceptance.time}</span></div>`
      :collab.code
        ?isCustomer
          ?`<label class="field acceptance-note"><span>${status==='rejected'?'补充验收意见':'验收意见'}</span><textarea id="acceptanceNote" rows="3" placeholder="通过时可填写说明；驳回时必须说明需要修改的内容"></textarea></label><div class="acceptance-actions"><span>${complete?'请确认查看完全部资料':'资料缺失时不能通过验收'}</span><button class="secondary" data-collab="rejectDelivery">驳回修改</button><button class="primary" data-collab="acceptDelivery" ${complete?'':'disabled'}>验收通过</button></div>`
          :'<div class="acceptance-waiting">交付资料已可查看，仅客户可以执行验收操作。</div>'
        :'<div class="acceptance-waiting">等待实施顾问和开发人员完成交付后再验收。</div>';
    return `<section class="acceptance-card ${status||'pending'}"><div class="acceptance-title"><div><span class="role-tag customer">客户验收包</span><h3>${status==='accepted'?'交付已验收':status==='rejected'?'交付已退回修改':collab.code?.file?'请核对全部交付成果':'验收资料准备中'}</h3><p>${status==='accepted'?`验收意见：${esc(collab.acceptance.note||'无')}`:status==='rejected'?`修改意见：${esc(collab.acceptance.note)}`:'查看需求、实施文档和开发代码后，再决定通过或驳回。'}</p></div><span class="acceptance-completeness ${complete?'complete':'incomplete'}">${complete?'3/3 资料齐全':`${[!!collab.requirement,!!collab.implementation?.file,!!collab.code?.file].filter(Boolean).length}/3 资料待补充`}</span></div><div class="acceptance-package">${acceptanceItem('01','需求方案',collab.requirement,'viewRequirement')}${acceptanceItem('02','实施文档',collab.implementation,'viewImplementation')}${acceptanceItem('03','开发代码',collab.code,'viewCode')}</div><div class="acceptance-test"><div><small>测试入口</small><b>${testUrl?esc(testUrl):'未提供（可选）'}</b></div>${testUrl?`<button class="secondary" data-collab="openTestUrl">打开测试页面</button>`:''}</div>${collab.code?`<div class="acceptance-version"><span><small>交付版本</small><b>${esc(collab.code.version||'未填写')}</b></span><span><small>提交时间</small><b>${esc(collab.code.time||'未记录')}</b></span><span><small>交付说明</small><b>${esc(collab.code.content||'未填写')}</b></span></div>`:''}${acceptanceAction}</section>`;
  }
  ...
  function showDoc(item,type){modal(`<div class="modal-head"><div><h2>${esc(item.title)}</h2><p>${type} · ${esc(item.version||'已确认版本')}</p></div><button data-close>×</button></div><div class="document-preview"><h3>${esc(item.title)}</h3><p>${esc(item.content||'未填写详细说明')}</p>${attachedFile(item.file)}<h4>交付状态</h4><p>${item.file||type==='需求'?'内容已提交':'已有文字说明，但缺少正式上传文件。'}</p></div><div class="modal-actions"><button class="secondary" data-project-id="${esc(collab.projectId)}">返回项目</button><button class="secondary" data-close>关闭</button>${item.file?.id?`<button class="primary" data-collab="downloadFile" data-file-id="${item.file.id}">下载文件</button>`:''}</div>`)}
  ...
  document.addEventListener('click',async e=>{
    ...
    if(action==='submitCode'){
      ...
      b.disabled=true;b.textContent='正在保存…';
      try{const saved=file?await persistFile(file):existing;const item={title,version:document.querySelector('#codeVersion').value.trim()||'V1.0.0',testUrl:document.querySelector('#codeTestUrl')?.value.trim()||'',content:document.querySelector('#codeNote').value.trim()||`已提交开发交付包：${saved.name}`,file:saved,time:now()};collab.code=item;collab.codeVersions.push({...item,note:item.content});collab.acceptance=null;recordEvent('提交开发交付',`${item.version} · ${saved.name}`);store();toast('开发成果已提交，等待客户验收')}catch(error){b.disabled=false;b.textContent='重新提交';toast('文件保存失败，请重试')}return;
    }
    ...
    if(action==='acceptDelivery'){
      if(collab.role!=='customer')return toast('请切换到客户视角后执行验收');
      if(!collab.requirement||!collab.implementation?.file||!collab.code?.file)return toast('需求、实施文档和开发代码齐全后才能通过验收');const note=document.querySelector('#acceptanceNote')?.value.trim()||'';collab.acceptance={status:'accepted',note,time:now()};recordEvent('客户验收通过',collab.code.version||collab.code.title);store();toast('交付已验收并归档');return;
    }
    ...
    if(action==='openTestUrl'){const url=collab.code?.testUrl||'';if(!/^https?:\/\//i.test(url))return toast('测试地址格式不正确');window.open(url,'_blank','noopener,noreferrer');return}
