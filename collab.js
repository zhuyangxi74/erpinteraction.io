(function(){
  ...
  function acceptanceItem(kind,title,item,action){const file=item?.file,detail=item?(file?.name||'在线内容'):'尚未提交';return `<article class="acceptance-item ${item?'ready':'missing'}"><i>${item?'✓':'○'}</i><div><small>${kind}</small><b>${title}</b><span>${esc(detail)}${item?.version?` · ${esc(item.version)}`:''}</span></div><div>${item?`<button class="secondary" data-collab="${action}">查看</button>${file?`<button class="link" data-collab="downloadFile" data-file-id="${file.id}">下载</button>`:''}`:'<em>等待提交</em>'}</div></article>`}
  ...
  window.openCustomerAssistant=()=>{collab.role='customer';store()};window.renderCurrentAcceptance=acceptancePanel;window.renderResearch=render;render();
})();
